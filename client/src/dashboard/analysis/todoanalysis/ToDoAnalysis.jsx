...
export default function ToDoAnalysis() {
  ...
  return (
    <div className="space-y-5">
      ...
      <nav className="hidden w-full items-center gap-1.5 overflow-x-auto rounded-2xl border border-amber-100/10 bg-black/20 p-1.5 scrollbar-none sm:flex">
        {TODO_ANALYSIS_SECTIONS.map((section) => {
          ...
          return (
            <Motion.button
              ...
              title={section.label}
              className={`flex min-w-[7rem] flex-1 items-center justify-center gap-2 overflow-hidden whitespace-nowrap rounded-xl px-3 py-2 text-xs font-semibold transition-colors ${
                isActive
            ...
            >
              ...
              <span className="min-w-0 truncate text-center">{section.label}</span>
            </Motion.button>
          );
        })}
      </nav>
