...
export default function CreateGoal() {
  ...
  const activeGoals = goals.filter((goal) => goal.deadline >= today);
  const archivedGoals = goals.filter((goal) => goal.deadline < today);
  ...
  const saveArchiveEdit = () => {
    ...
    setGoals((prev) =>
      prev.map((g) => g.id === archiveEditGoal.id ? { ...g, ...archiveEditForm, title: archiveEditForm.title.trim() } : g)
    );
    setGoalLogs((prev) => [
  ...
  const doArchiveDelete = () => {
    if (!archiveDeleteId) return;
    handleDelete(archiveDeleteId);
